...
class DeleteOldProcessInstances(
    ...
    val logger = LoggerFactory.getLogger(DeleteOldProcessInstances::class.java)
    var running = true
    ...
    fun execute(processInstanceId: Long, date: Long) {

        val statements: MutableList<String> = mutableListOf()
        ScriptUtils.splitSqlScript("/delete_scenario.sql", ";", statements)
        statements.forEach {statement -> jdbcTemplate.update(statement, processInstanceId, date) }
    ...
    private fun migrate(throughput: Throughput) {
        ...
        do {
            ...
            try {
                migratedCount = count.block()?.toInt()
