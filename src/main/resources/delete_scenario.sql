...
DELETE FROM ARCH_CONTRACT_DATA a WHERE NOT EXISTS (
...
DELETE FROM ARCH_DATA_INSTANCE a WHERE NOT EXISTS (
SELECT id FROM ARCH_PROCESS_INSTANCE b
WHERE a.CONTAINERID = b.ROOTPROCESSINSTANCEID
AND a.CONTAINERTYPE = 'PROCESS_INSTANCE'
AND a.tenantId = ? AND b.tenantId = ?);

-- ARCH_DOCUMENT_MAPPING
-- Depends on PROCESS INSTANCE (PROCESSINSTANCEID)

DELETE FROM ARCH_DOCUMENT_MAPPING a WHERE NOT EXISTS (
SELECT ID FROM ARCH_PROCESS_INSTANCE b
...
DELETE FROM ARCH_FLOWNODE_INSTANCE a WHERE NOT EXISTS (
SELECT id FROM ARCH_PROCESS_INSTANCE b
...
DELETE FROM ARCH_PROCESS_COMMENT a WHERE NOT EXISTS (
SELECT ID FROM ARCH_PROCESS_INSTANCE b
WHERE a.PROCESSINSTANCEID = b.SOURCEOBJECTID
AND a.tenantId = ? AND b.tenantId = ?);
--
...
DELETE FROM ARCH_REF_BIZ_DATA_INST a WHERE NOT EXISTS (
SELECT ID FROM ARCH_PROCESS_INSTANCE b
...
DELETE FROM ARCH_CONNECTOR_INSTANCE a WHERE NOT EXISTS (
SELECT ID FROM ARCH_PROCESS_INSTANCE b
WHERE a.CONTAINERID = b.ROOTPROCESSINSTANCEID
AND a.CONTAINERTYPE = 'process'
AND a.tenantId = ? AND b.tenantId = ?);
...
AND a.CONTAINERTYPE = 'ACTIVITY_INSTANCE'
AND a.tenantId = ? AND b.tenantId = ?);
...
DELETE FROM ARCH_MULTI_BIZ_DATA a WHERE NOT EXISTS (
